    ...
    <div className="pt-4 pb-9 px-4 shadow rounded-2xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
        {weekDates.map((date, index) => {
          ...
          const dayData = weekData[dayName] || { color: "", subColor: "", spacing: "", courses: [] };
                  ...
                  {courses.length > 0 ? (
                    courses.map((course, i) => (
                      <div key={i} className="flex flex-col -mb-3">
                        <div className={`z-10 relative space-y-1 text-sm px-2 py-4 rounded-xl mt-1 ${color}`}>
                        ...
                        <div className={`relative top-[-1rem] pt-5 pb-1 flex items-center justify-end
                           rounded-2xl mb-3 ${subColor}`}>
                          <div className="w-full text-xs text-center">{course.code}</div>
                        </div>
                      </div>
                    ))
                  ) : (
